...
try:
    while True:
        # if ("Saturday" == x.strftime("%A")):
        if (CurrentWeek % 2 == 0):  # recycle & normal bin when the week of the year is odd
            GPIO.output(26, True)
            GPIO.output(16, True)
            time.sleep(30)

        else:  # normal bin only when the week of the year is even
            GPIO.output(16, True)
            time.sleep(30)
